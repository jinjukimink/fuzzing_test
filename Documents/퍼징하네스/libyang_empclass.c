...
int LLVMFuzzerTestOneInput(uint8_t const *buf, size_t len) {
  ...
  lyd_parse_data_mem(ctx, data, LYD_JSON, LYD_PARSE_ONLY, LYD_VALIDATE_PRESENT, &tree_in);
  // ly_in_new_memory(data, &in); // The root cause of the double free

  lyd_parse_op(ctx, tree_in, NULL, LYD_JSON, LYD_TYPE_DATA_YANG, &tree_out, &tree);

  lyd_free_all(tree_in);
  lyd_free_all(tree_out);
  // ly_in_free(in, true); // The root cause of the double free
  ly_ctx_destroy(ctx);
